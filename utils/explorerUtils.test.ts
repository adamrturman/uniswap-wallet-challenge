...
describe('explorerUtils', () => {
  ...
  describe('openExplorer', () => {
    it('should open explorer URL for valid transaction hash and chain', async () => {
      ...
      expect(mockOpenURL).toHaveBeenCalledWith('https://etherscan.io/tx/0x1234567890abcdef');
    });
    ...
    it('should open different explorer URLs for different chains', async () => {
      ...
      expect(mockOpenURL).toHaveBeenCalledWith('https://polygonscan.com/tx/0xabcdef1234567890');

      await openExplorer('0x9876543210fedcba', 'Arbitrum');
      expect(mockOpenURL).toHaveBeenCalledWith('https://arbiscan.io/tx/0x9876543210fedcba');
    });
    ...
    it('should throw error when chain key is empty', async () => {
      await expect(openExplorer('0x1234567890abcdef', '' as any)).rejects.toThrow(
        'Transaction hash and chain key are required'
      );
    });

    it('should throw error when chain key is null', async () => {
      await expect(openExplorer('0x1234567890abcdef', null as any)).rejects.toThrow(
        'Transaction hash and chain key are required'
      );
    });

    it('should throw error when chain key is undefined', async () => {
      await expect(openExplorer('0x1234567890abcdef', undefined as any)).rejects.toThrow(
        'Transaction hash and chain key are required'
      );
    });

    it('should throw error when chain config is not found', async () => {
      await expect(openExplorer('0x1234567890abcdef', 'UnknownChain' as any)).rejects.toThrow(
        'Explorer URL not found for chain: UnknownChain'
      );
    });
    ...
    it('should handle Linking.openURL errors', async () => {
      ...
      await expect(openExplorer('0x1234567890abcdef', 'Ethereum')).rejects.toThrow(
        'Failed to open URL'
      );
    });
