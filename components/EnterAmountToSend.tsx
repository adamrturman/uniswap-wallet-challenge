...
export default function EnterAmountToSend({
  ...
  const showError = useMemo(() => {
    const numAmount = parseFloat(amount);
    if (amount.trim().length === 0 || numAmount <= 0 || isNaN(numAmount))
      return false;
      ...
      if (wallet && recipientAddress) {
        try {
          ...
          if (selectedToken.tokenKey === 'native') {
            const { estimateGasForTransaction } = await import(
            ...
            >
              ...
              <View style={styles.balanceRow}>
                ...
                <View style={styles.balanceMaxSection}>
                  ...
                  >
                    {formatTokenAmount(selectedToken.balance)}{' '}
                    {selectedToken.symbol}
                  </Text>
                  <TouchableOpacity
                    style={[
                  ...
                  >
                    <Text
                      style={[styles.maxButtonText, { color: colors.primary }]}
                    >
                      Max
                    </Text>
                  </TouchableOpacity>
